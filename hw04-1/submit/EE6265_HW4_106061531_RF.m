...
gg = zeros(size(channel_data_new, 1), Nbeam); % sample x element
%gg = zeros(size(channel_data_new)); % sample x element
    ...
    for iSample = 1:Nsample
        ...
        for iElement = 1:Nelement
            ...
            if (idx >= 1) && (idx <= Nsample)
                ...
                %gg(iSample, iElement) = channel_data_new(idx,iElement);
                gg(iSample, iBeam) = channel_data_new(idx,iElement);
            end
...
colorbar;
colormap(gray);
saveFig(fig, [save_path 'off-axis.pdf'])
...
axial_res_6 = (locs - axial_locs_6(1:2:6)) * dx;
axial_res_20 = (locs - axial_locs_20(1:2:6)) * dx;
...
lateral_res_6 = (locs - axial_locs_6(1:2:6)) * dz;
lateral_res_20 = (locs - axial_locs_20(1:2:6)) * dz ;


lateral_res_gt_6 = (dsin_theta * locs) * dz;
